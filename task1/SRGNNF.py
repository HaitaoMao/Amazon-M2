...
class SRGNNF(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.item_embedding = nn.Embedding(
            self.n_items, self.embedding_size // 2, padding_idx=0
        )
        ...
        self.linear_transform = nn.Linear(
            self.embedding_size * 2, self.embedding_size // 2, bias=True
        )
    ...
    def _reset_parameters(self):
        stdv = 1.0 / math.sqrt(self.embedding_size*2)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        feature_table = feature_table.squeeze()
        table_shape = feature_table.shape
        # import ipdb; ipdb.set_trace()
        hidden = torch.cat((hidden, feature_table), dim=-1)
        # import ipdb; ipdb.set_trace()
