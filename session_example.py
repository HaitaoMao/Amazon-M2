...
def get_args():
    ...
    parser.add_argument(
        "--train_batch_size", type=int, default=256, help="train batch size."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.model.endswith('F') and args.model!='GRU4RECF':
        model = args.model[:-1]
    else:
        model =args.model
    config = Config(
        model=model, dataset=f"{args.dataset}", config_dict=config_dict
    )
    ...
    trainer.resume_checkpoint('./saved/CORE-May-26-2023_17-07-43.pth')
    all_indices = []
    ...
    #  OrderedDict([('recall@100', 0.5798), ('mrr@100', 0.1568)])   
